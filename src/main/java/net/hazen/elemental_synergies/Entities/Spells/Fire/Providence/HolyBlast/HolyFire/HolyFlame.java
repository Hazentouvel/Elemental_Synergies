...
public class HolyFlame extends AbstractMagicProjectile implements GeoEntity {
    private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
    private int spellLevel;
    private float spellPower;

    public HolyFlame(EntityType<? extends Projectile> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
        this.setNoGravity(true);
    }

    public HolyFlame(Level level, LivingEntity shooter)
    {
        this(ESEntityRegistry.HOLY_FLAME.get(), level);
        setOwner(shooter);
    ...
    public void trailParticles() {
        ...
        level.addParticle(ESParticleHelper.HOLY_EMBER_PARTICLE, pos.x, pos.y, pos.z, random.x, random.y, random.z);
    }

    @Override
    public void impactParticles(double x, double y, double z) {
        MagicManager.spawnParticles(level, ESParticleHelper.HOLY_EMBER_PARTICLE, x, y, z, 12, .08, .08, .08, 0.3, false);
    }
