package net.hazen.elemental_synergies.Entities.Spells.Fire.SupremeCalamitas.BrimstoneHellblast;
...
public class BrimstoneHellblast extends AbstractMagicProjectile implements GeoEntity {
    ...
    public BrimstoneHellblast(Level pLevel, LivingEntity pShooter) {
        this(ESEntityRegistry.BRIMSTONE_HELLBLAST.get(), pLevel);
        ...
        if (!this.level.isClientSide) {
            ...
            for (Entity entity : entities) {
                ...
                if (distanceSqr < (double) explosionRadiusSqr
                    ...
                    DamageSource ritual = entity.level().damageSources().source(
                            ritualHolder.unwrapKey().orElseThrow(),
                            this.getOwner()
                    );

                    Holder<DamageType> bloodHolder =
                            DamageSources.getHolderFromResource(entity, ISSDamageTypes.BLOOD_MAGIC);

                    DamageSource blood = entity.level().damageSources().source(
                            bloodHolder.unwrapKey().orElseThrow(),
                            this.getOwner()
                    );
            ...
            if ((Boolean)ServerConfigs.SPELL_GREIFING.get()) {
                Explosion explosion = new Explosion(this.level, (Entity)null, ((AbstractSpell) ESSpellRegistries.BRIMSTONE_HELLBLAST.get()).getDamageSource(this, this.getOwner()), (ExplosionDamageCalculator)null, this.getX(), this.getY(), this.getZ(), this.getExplosionRadius() / 4.0F, true,
            ...
            PacketDistributor.sendToPlayersTrackingEntity(this, new BrimstoneExplosionParticlesPacket(hitResult.getLocation().subtract(this.getDeltaMovement().scale((double)0F)), this.getExplosionRadius()), new CustomPacketPayload[0]);
