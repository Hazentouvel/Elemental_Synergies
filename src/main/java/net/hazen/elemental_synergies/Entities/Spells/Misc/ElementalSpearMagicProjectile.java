...
public class ElementalSpearMagicProjectile extends AbstractMagicProjectile {
    ...
    public ElementalSpearMagicProjectile(EntityType<? extends Projectile> type, Level level) {
        super(type, level);
    }
    ...
    public AnimationState getAnimationState(String input) {
        if (input == "spawn") {
            ...
            return input == "idle" ? this.idlespawnAnimationState : new AnimationState();
        }
    }
    ...
    protected void onHit(HitResult result) {
        ...
        if (hitresult$type == Type.ENTITY) {
            ...
            this.level().gameEvent(GameEvent.PROJECTILE_LAND, result.getLocation(), Context.of(this, (BlockState)null));
    ...
    public void checkDespawn() {
        if (this.level().getDifficulty() == Difficulty.PEACEFUL) {
            ...
            Entity entity = this.level().getNearestPlayer(this, (double)-1.0F);
            if (entity != null) {
    ...
    protected void setRotationFromVector(Vec3 vec) {
        if (vec == null) return;
        double dx = vec.x;
        double dz = vec.z;
        double dy = vec.y;
        ...
        this.setYRot(yaw);
        this.setXRot(pitch);
        // Update previous rotation values to avoid interpolation jumps
        this.yRotO = this.getYRot();
        this.xRotO = this.getXRot();
    }

    static {
        STATE = SynchedEntityData.defineId(ElementalSpearMagicProjectile.class, EntityDataSerializers.INT);
    }
}
